...
async def wait_for_images(page, timeout=25):
    try:
...
async def process_url(browser, url_to_scan, captured_domains):
...
async def get_domains(request: Request):
    ...
    async with await launch_async(
        ...
        await asyncio.gather(*(process_url(browser, u, captured_domains) for u in urls))
